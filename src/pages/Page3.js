...
export class Page3 extends PageBase {
    constructor(w, h) {
        ...
        const assets = [
            ...
            { url: '/assets/images/page3/1.png', x:  w * 0.4,  y: 0,        z:  h * 0.2, scale: 0.2 }  // 1 paling pojok kanan depan
        ];
    ...
    _generatePositions(count) {
        ...
        // Atas diturunin sedikit (y parameter)
        const yAtas = H * 0.35;
        const yBawah = 0; 
        
        // Atas dimundurin (z parameter)
        const zBawah = H * 0.45;
        const zAtas = H * 0.20;

        if (count > 0) positions.push({ x: col1, y: yBawah, z: zBawah }); // 0: Kiri Bawah
        if (count > 1) positions.push({ x: col2, y: yBawah, z: zBawah }); // 1: Tengah Bawah
        if (count > 2) positions.push({ x: col2, y: yAtas,  z: zAtas }); // 2: Tengah Atas
        if (count > 3) positions.push({ x: col3, y: yBawah, z: zBawah }); // 3: Kanan Bawah
        if (count > 4) positions.push({ x: col1, y: yAtas,  z: zAtas }); // 4: Kiri Atas
